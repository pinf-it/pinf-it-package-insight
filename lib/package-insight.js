...
exports.makeMergeHelpers = function(exports, descriptor, copied) {
	...
	var helpers = {
		...
		mergeObjectTo: function(key, targetKey) {
			if (descriptor.raw[key] && typeof descriptor.raw[key] === "object") {
...
function normalize(descriptorPath, descriptor, options, callback) {
	...
	try {
		...
		helpers.objectToObject("engines", ["config", "engines"]);
		helpers.mergeObjectTo("engine", ["config", "engines"]);
		...
		if (options.type === "component") {
			...
			helpers.objectToObject("scripts", ["config", "scripts"]);
		}

		helpers.objectToObject("directories", ["config", "directories"]);
		...
		function detectBundledDependencies(callback) {
			if (
				...
				return FS.exists(PATH.join(PATH.dirname(options._realpath(descriptorPath)), descriptor.normalized.config.directories.dependency), function(exists) {
					...
					return FS.readdir(PATH.join(PATH.dirname(options._realpath(descriptorPath)), descriptor.normalized.config.directories.dependency), function(err, filenames) {
						if (err) return callback(err);
						filenames.forEach(function(filename) {
							descriptor.normalized.dependencies.bundled[filename] = "./" + descriptor.normalized.config.directories.dependency + "/" + filename;

							var shasum = CRYPTO.createHash("sha1");
							shasum.update(PATH.join(options.packagePath, descriptor.normalized.config.directories.dependency, filename));
		...
		function extraNormalization(callback) {
				...
				if (property === "config.directories") {
					if (
						...
						for (var type in descriptor.normalized.config.directories) {
							descriptor.normalized.config.directories[type] = helpers.prefixRelativePath(descriptor.normalized.config.directories[type]).replace(/\/$/, "")
						}
					}
				} else
			...
			return normalize("exports.main", function(err) {
				if (err) return callback(err);
				return normalize("config.directories", function(err) {
					if (err) return callback(err);
