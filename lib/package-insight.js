...
exports.parse = function(packagePath, options, callback) {
	try {
		...
		var env = PINF_PRIMITIVES.normalizeEnvironmentVariables(env, {
			PINF_PROGRAM: PATH.join(options._realpath(packagePath), "program.json"),
			PINF_PACKAGE: PATH.join(options._realpath(packagePath), "package.json")
		});
		...
		(options.lookupPaths || exports.LOOKUP_PATHS).forEach(function(resolve) {
			...
			waitfor(function(done) {
				return exports.parseDescriptor(filename, opts, function(err, descriptor) {
					...
					if (Object.keys(descriptor.raw).length === 0) return done(null);
