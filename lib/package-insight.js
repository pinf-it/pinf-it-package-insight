...
exports.parse = function(packagePath, options, callback) {
	try {
		...
		ASSERT(FS.existsSync(options._realpath(packagePath)));
		...
		[
			...
			waitfor(function(done) {
				return exports.parseDescriptor(PATH.join(packagePath, filename), options, function(err, descriptor) {
					...
					packageDescriptor.combined.dirpath = packagePath;
					var shasum = CRYPTO.createHash("sha1");
					shasum.update(packagePath);
					packageDescriptor.combined.id = shasum.digest("hex") + "-" + PATH.basename(packagePath);
