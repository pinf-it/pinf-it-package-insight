		...
		function detectBundledDependencies(callback) {
			if (
				...
				return FS.exists(PATH.join(PATH.dirname(descriptorPath), "node_modules"), function(exists) {
					...
					return FS.readdir(PATH.join(PATH.dirname(descriptorPath), "node_modules"), function(err, filenames) {
						if (err) return callback(err);
						filenames.forEach(function(filename) {
							descriptor.normalized.dependencies.bundled[filename] = "./node_modules/" + filename;
						});
						return callback(null);
					});
				});
